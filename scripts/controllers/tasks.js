...
// document.body.querySelector("#app").classList = "bg-gray-100 min-h-screen flex";
...
//   document.getElementById("create-task-modal").className =
...
//   card.innerHTML = `<td class="px-6 py-4">
//             <p>${task.title}</p>
...
//     e.preventDefault();
//     await storage.deleteTask(actualUser, task.id);
...
//     document.getElementById("edit-task-modal").className =
//       "bg-gray-100 min-h-screen absolute inset-0 bg-black bg-opacity-60";
//   });
...
//     await storage.updateTask(actualUser, task.id, { status: nextStatus });
//     task = storage.getTask(actualUser);
//     renderTasks(tasks);
//   });
...
//   await storage.saveTask(actualUser, task);
//   tasks = storage.getTask(actualUser);
//   renderTasks(tasks);
// }
// // Render Tasks
...
//   };
//   // Validate
//   if (!task.title || !task.frequency || !task.priority || !task.dueDate) return;
//   createTask(task);
//   tasksForm.reset();
// });
...
//   await storage.updateTask(actualUser, editingTaskId, {
//     title,
//     description,
//     category,
//     frequency,
